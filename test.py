	...
	for idx, img_name in enumerate(sorted(os.listdir(img_path))):
		...
		for key in bboxes:
			...
			for jk in range(new_boxes.shape[0]):
				...
				cur_prob.append(100*new_probs[jk])
				all_dets.append((key,100*new_probs[jk]))
				print(key, 100*new_probs[jk], real_x1, real_y1, real_x2, real_y2)
				if (key == 'Bearing'):
					Bear = Bear + 1
				elif(key == 'OutofPlaneStiffener'):
					Outof = Outof + 1
				elif(key == 'GussetPlateConnection'):
					Guss = Guss + 1
				else:
					Cover = Cover + 1
		...
		writer.writerow({'Imagename':initial, 'Bear': Bear, 'Outof': Outof, 'Guss': Guss, 'Cover':Cover,'Prob':sum(cur_prob)})
